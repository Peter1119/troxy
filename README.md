...
| iOS Simulator / 실기 | ① 기기를 `127.0.0.1:8080` 프록시로 연결 → ② Safari에서 `http://mitm.it` → iOS 프로필 다운로드 → ③ **설정 → 일반 → VPN 및 기기 관리** → mitmproxy 프로필 설치 → ④ **설정 → 일반 → 정보 → 인증서 신뢰 설정**에서 mitmproxy 토글 ON |
| Android Emulator | 프록시 연결 후 `http://mitm.it` 에서 `.cer` 다운로드 → Settings → Security → Install from storage. **Android 7+ 는 아래 snippet 추가 필요** |

<details>
<summary>Android 7+ : <code>network_security_config.xml</code> 샘플</summary>
...
> Android 14+ : 프로덕션 서명 앱에서는 user CA 신뢰가 제한됩니다. debug 빌드에서만 가로채는 걸 권장.
...
```bash
troxy start           # proxy 시작 — 별도 창에서 유지 (Ctrl+C로 종료)
# (앱/기기에서 트래픽 발생시키기)
...
troxy mock add --name user-500 -d api.example.com -p "/api/users/*" -s 500 \
  --body '{"error": "boom"}'
troxy mock disable user-500                     # 비활성화
troxy mock enable user-500                      # 재활성화
troxy mock list
```
